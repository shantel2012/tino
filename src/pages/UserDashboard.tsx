...
type ParkingSpace = {
  ...
  features: string[];
  distance: string;
  availability: string; // e.g., "Available now", "Available from 2 PM"
};
...
type Booking = {
  ...
  price: number;
  spaceId?: number; // for linking to /parking/:id (optional)
};

type Favorite = number; // ParkingSpace.id
...
const UserDashboard: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"search" | "bookings" | "favorites">(
    "search"
  );

  // Search filters
  const [searchQuery, setSearchQuery] = useState("");
  const [dateFilter, setDateFilter] = useState(""); // not fully used yet; placeholder for future availability search
  const [priceFilter, setPriceFilter] = useState(""); // "0-10" | "10-20" | "20+"

  // Favorites (store IDs)
  const [favorites, setFavorites] = useState<Favorite[]>([1, 3]); // pre-favorited demo
  ...
  const filteredSpaces = useMemo(() => {
    ...
    // Date filter placeholder: you'd filter by availability windows here.
    // For demo, if a date is chosen, we'll keep all results (no-op).
    return spaces;
